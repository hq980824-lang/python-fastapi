...
from src.modules.posts.post_dto import (
    ImportResult,
    PostBatchCreate,
    PostCreate,
    PostResp,
...
async def get_all_posts(
    db: DbDep, params: PageQuery = Depends(), author_id: int | None = None
):
...
async def update_post(
    payload: PostUpdate, db: DbDep, db_post: PostDB = Depends(get_own_post)
):
...
async def create_batch_posts(
    payloads: PostBatchCreate, current_user: CurrentUser, db: DbDep
):
...
@router.post(
    "/import", response_model=ImportResult, dependencies=[Depends(get_current_user)]
)
